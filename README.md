...
```bash
sentiment-analysis-lambda$ sam build
```
...
```bash
sentiment-analysis-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sentiment-analysis-lambda$ sam local start-api
sentiment-analysis-lambda$ curl http://localhost:3000/
```
...
```bash
sentiment-analysis-lambda$ sam logs -n HelloWorldFunction --stack-name sentiment-analysis-lambda --tail
```
...
```bash
sentiment-analysis-lambda$ pip install pytest pytest-mock --user
sentiment-analysis-lambda$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sentiment-analysis-lambda
```
